...
const luaScript = require('./luascript')(redisClient);
...
function buildLuckyMoneyArr(opts, cb){
  ...
  if(opts.ltype=='fixed'){
      for(var i = 0; i < opts.quantity; i++){
          arr.push(JSON.stringify({money:Utils.strip(opts.amount)}));
      }
      return cb(arr);
      ...
      for(var i = 0; i < opts.quantity - 1; i++){
          ...
          usedMoney += t
          arr.push(JSON.stringify({money:Utils.strip(t / 1000000)}));
      }
      arr.push(JSON.stringify({money:Utils.strip((money - usedMoney) / 1000000)}));
      return cb(arr);
  }else{
...
function pushLuckyMoney(luckymoneyId,data,cb){
  redisClient.lpush(LM_QUEUE_SURPLUS+luckymoneyId,data,function(err,res){
      if(res>0){
        redisClient.EXPIRE(LM_QUEUE_SURPLUS+luckymoneyId,24*60*60);
        return cb({status:0}); 
      } else{
        return cb({status:4});
      }  
...
WalletService.getInstanceWithAuth = function(opts, cb) {
  function withSignature(cb) {
    ...
    console.log('opts.copayerId:',opts.copayerId)
 ...
 */
WalletService.prototype.buildLuckyMoneyOrder = function(userinfo,opts, cb) {
  if (!checkRequired(opts, ['quantity'], cb)) return;
  if (!checkRequired(opts, ['amount'], cb)) return;
  if (!checkRequired(opts, ['ltype'], cb)) return;

  var self = this;
  var theDate = new Date();
  var luckymoneyId = uuidv1();
  theDate.setHours(theDate.getHours() + 24);
  var luckymoney ={
    luckymoneyId: luckymoneyId,
    fromOpenId: userinfo.openid,
    fromNickname: userinfo.nickname,
    fromHeadImg: userinfo.headimgurl,
    amount: opts.amount,
    quantity: opts.quantity,
    surplusQuantity: opts.quantity,
    comment: opts.comment||"",
    ltype: opts.ltype,
    items:[],
    createdOn: Math.floor(Date.now() / 1000),
    txid:'',
    expiredOn: theDate.setHours(theDate.getHours() + 24)
  }
  self.storage.storeLuckyMoney(luckymoney,function(err,result) {
    if(err) return cb(null, {status:'failed'})
    return cb(null,{luckymoneyId:luckymoneyId,address:"1N5Y2z1wKLjVTDYgNjfW1iH9DTGTsySNBv"});
  });
 ...
 * @param {string} opts.openId 
 */
WalletService.prototype.payLuckyMoneyOrder = function(userinfo,opts, cb) {
  if (!checkRequired(opts, ['luckymoneyId'], cb)) return;
  if (!checkRequired(opts, ['txid'], cb)) return;
  var self = this;
  this.storage.payLuckyMoney(userinfo.openid,opts, function(err,result){
    log.info('..........result '+JSON.stringify(result));
    if (result.status==0) {
      self.storage.getLuckyMoney(opts.luckymoneyId, function(err,res){
        log.info('..........res '+JSON.stringify(res));
        buildLuckyMoneyArr({quantity:res.quantity ,amount:res.amount ,ltype:res.ltype},function(result){
          log.info('..........result '+JSON.stringify(result));
          if(_.isArray(result)){
            // 推送到队列
            pushLuckyMoney(opts.luckymoneyId,result,function(rr){
              log.info('..........rr '+JSON.stringify(rr));
              return cb(rr);
            });
          }
        });
 ...
 * 查询红包 TODO
 */
WalletService.prototype.quryLuckyMoneyOrder = function(userinfo,opts,cb){
  if (!checkRequired(opts, ['ltype'], cb)) return;
  this.storage.quryLuckyMoneyOrder(userinfo.openid,opts.ltype,function(err,res){
 ...
 * 抢红包
 */
WalletService.prototype.scrambleLuckymoney = function(userinfo,lmId, cb){
  if (_.isEmpty(lmId)) return cb(null);
  var self = this;
  this.storage.getLuckyMoney(lmId,function(err,res){
    var result = {
      luckymoney:res,
      userinfo:userinfo,
      status:0,
    ...
    result.status = 1;
    result.msg = "红包不存在";
    if(_.isEmpty(res)) return cb(result);

    // 抢红包
    luaScript.run('grabbingRedPacket',
      LM_QUEUE_SURPLUS+lmId,
      LM_QUEUE_USED+lmId,
      userinfo.openid
      ).then(res => {
        log.info('....................scrambled res: '+res);
        var jsonRes = JSON.parse(res);
        if(jsonRes.status==='again'){
          // 已抢过
          result.status =2;
          result.msg = "已抢过";
          result.yours = jsonRes.money;
          return cb(result);
        }else if(jsonRes.status==='expired'){
          // 已过期
          result.status =3;
          result.msg = "已过期";
          return cb(result);
        }else if(jsonRes.status==='got'){
          // 抢到
          var lm = JSON.parse(res);
          var item = {
            userId:userinfo.openid,
            nickname:userinfo.nickname,
            headimgurl:userinfo.headimgurl,
            amount:lm.money,
            createOn:moment().format('L'),
            withdraw:'no'
          }
          result.yours = lm.money;
          self.storage.updateScrambledLuckyMoney({luckymoneyId:lmId,item:item},function(err,res){
            log.info('....................updateScrambledLuckyMoney '+JSON.stringify(res))
            if(res){
              result.status =0;
              result.luckymoney = res;
              return cb(result);
            }else{
              log.info('....................updateScrambledLuckyMoney err '+err)
              result.status =5;
              result.msg = "领取失败";
              return cb(result);
            }
 ...
 * 查看红包
 */
WalletService.prototype.findLuckyMoneyOrder = function(luckymoneyId,cb){
  if (_.isEmpty(luckymoneyId)) return cb(null);
  this.storage.findLuckyMoneyOrder(luckymoneyId,function(res){
    if(res) return cb(res);
    return cb({});
  });
}
 ...
 * @param {String} hash
 * @returns {JSONObject} transaction
 */
WalletService.prototype.getRawTransaction = function (hash, cb) {
    this.rpcClient.getRawTransaction(hash, 1, cb);
};
 ...
 */
WalletService.prototype.getNewAddress = function (cb) {
    this.rpcClient.getNewAddress(cb);
};
 ...
 * @returns {JSONObject} transaction
 */
WalletService.prototype.sendToAddress = function (toAddress, amount, cb) {
    this.rpcClient.sendToAddress(toAddress, amount, cb);
};
