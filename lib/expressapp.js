...
ExpressApp.prototype.start = function(opts, cb) {
  ...
  router.get('/luckymoney/prescramble/:lmid', function(req, res) {
    ...
    var url = wx.oauth.generateOAuthUrl('http://api.qyb.chainclub.one/bws/api/wxlogin/callback/?lmId='+lmId, 'snsapi_userinfo', 'custom_state');
    res.redirect(url);
  });
   ...
   * 返回：抢红包页面
   */
  router.get('/wxlogin/callback/:lmid', function(req, res) {
