...
ExpressApp.prototype.start = function(opts, cb) {
   ...
   *  其中，status值:0 成功，1 参数错误,3 系统错误 4 付款失败
   */
  router.post('/luckymoney/pay/', function(req, res) {
    getLMServerWithAuth(req, res, function(userinfo,server) {
      if(userinfo){
        server.payLuckyMoneyOrder(userinfo,req.body, function(err, response) {
          if (err) return returnError(err, res, req);
          res.json(response);
        });
      }else{
  ...
  router.get('/luckymoney/prescramble/:lmid', function(req, res) {
    ...
    var url = wx.oauth.generateOAuthUrl('http://luckymoney.let5see.xyz/bws/api/luckymoney/scramble/'+lmId, 'snsapi_userinfo', 'custom_state');
    res.redirect(url);
  });
   ...
   * 返回：抢红包页面
   */
  router.get('/luckymoney/scramble/:lmid', function(req, res) {
