...
Storage.prototype._createIndexes = function() {
  ...
  this.db.collection(collections.LUCKYMONEY).createIndex({
    luckymoneyId: 1
  }, { unique: true } );
};
...
//@empty add for fetch luckymoney
Storage.prototype.storeLuckyMoney = function(luckymoney, cb) {
  this.db.collection(collections.LUCKYMONEY).insert(luckymoney, {
    w: 1
  }, cb);
};

Storage.prototype.payLuckyMoney = function(openid,opts, cb) {
  var res = this.db.collection(collections.LUCKYMONEY).findAndModify(
    {luckymoneyId:opts.luckymoneyId, txid:"", fromOpenId:openid},
    [],
    {"$set":{"txid":opts.txid}},
    {
      "upsert": false
...
Storage.prototype.updateScrambledLuckyMoney = function(opts, cb) {
  var res = this.db.collection(collections.LUCKYMONEY).findAndModify(
    {luckymoneyId:opts.luckymoneyId},
    [],
    {"$push":{"items":opts.item}, "$inc": {"surplusQuantity": -1}},
    {
      "upsert": false,
      "new":true
    }
...
Storage.prototype.getLuckyMoney = function(luckymoneyId, cb) {

  this.db.collection(collections.LUCKYMONEY).findOne({luckymoneyId:luckymoneyId}, function (err, result) {
    if (err) cb(null, null);
    return cb(null, result);
  });
};

Storage.prototype.getToken = function (openid, cb) {
  this.db.collection(collections.ACCESS_TOKEN).findOne({openid:openid}, function (err, result) {
    if (err) throw err;
    return cb(null, result);
  });
};

Storage.prototype.setToken = function (openid, token, cb) {
  // 有则更新，无则添加
  var query = {openid: openid};
  var options = {upsert: true};

  this.db.collection(collections.ACCESS_TOKEN).update({ 
    openid: openid
  }, {
    "$set":
    { 
...
Storage.prototype.fetchLuckyMoneyByOpenId = function(openId, luckymoneyId, cb) {
  this.db.collection(collections.LUCKYMONEY).find({
    luckymoneyId: luckymoneyId,
    "items": {"$elemMatch": { "userId": openId} },
  },{"items.amount":1}).sort({
    luckymoneyId: 1
  }).toArray(function(err, result) {
    if (err) return cb(err);
...
Storage.prototype.quryLuckyMoneyOrder = function(openId, ltype, cb) {
  if(ltype=='received'){
    this.db.collection(collections.LUCKYMONEY).find({
      "items": {"$elemMatch": { "userId": openId} },
    },{"fromNickname":1,"amount":1, "quantity":1,"surplusQuantity":1, "comment":1, "luckymoneyId":1,"items":1}).sort({
      createdOn: 1
    }).toArray(function(err, result) {
      log.info('.........result')
...
Storage.prototype.findLuckyMoneyOrder = function(luckymoneyId,cb){
  this.db.collection(collections.LUCKYMONEY).findOne({luckymoneyId:luckymoneyId}, function (err, result) {
    if (err) throw err;
    return cb(result);
  });
